...
class Opmd2VTK:
        ...
        if self.CommonMesh:
            ...
            for fld in flds:
                ...
                if field_type=='vector':
                    self._convert_field_vec_full(fld) 
                elif field_type=='scalar':
                    self._convert_field_scl(fld)
            # write VTK file
            ...
            for fld in flds:
                ...
                if field_type=='vector':
                    ...
                    for comp in comps:
                        ...
                        self._convert_field_vec_comp(fld, comp)
                        # write VTK file
    ...
    def _convert_field_vec_full(self, fld):
        ...
        indx = self.grid.point_data.add_array( flds )
        self.grid.point_data.get_array(indx).name = fld
    ...
    def _convert_field_vec_comp(self, fld, comp):
        ...
        fld_data, self.info = get_field(fld, comp=comp)
        make_mesh()
        indx = self.grid.point_data.add_array(fld_data)
        self.grid.point_data.get_array(indx).name = fld+comp
